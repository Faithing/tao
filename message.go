...
var (
	...
	messageRegistry map[uint64]handlerUnmarshaler
)
...
func Register(msgType uint64, unmarshaler func([]byte) (Message, error), handler func(context.Context, WriteCloser)) {
...
func GetUnmarshalFunc(msgType uint64) UnmarshalFunc {
...
func GetHandlerFunc(msgType uint64) HandlerFunc {
	entry, ok := messageRegistry[msgType]
	if !ok {
		return nil
	}
...
type Message interface {
	RequestName() string
	ResponseName() string
	RequestCommand() uint64
	ResponseCommand() uint64
	Serialize() ([]byte, error)
	SetCustom(v interface{})
	//buf ([]byte)
	//var buf []byte = builder.FinishedBytes()
}
...
func (hbm HeartBeatMessage) RequestCommand() uint64 {
	return HeartBeat
}

// ResponseCommand returns message number.
func (hbm HeartBeatMessage) ResponseCommand() uint64 {
	return HeartBeat
}
